...
router.get("/collections", async (_req, res): Promise<void> => {
  ...
  const counts = await db.select({
    collectionId: productCollectionsTable.collectionId,
    count: sql<number>`count(*)::int`,
  }).from(productCollectionsTable).groupBy(productCollectionsTable.collectionId);
...
router.put("/collections/:id", async (req, res): Promise<void> => {
  ...
  const data = parsed.data;
  const [collection] = await db.update(collectionsTable).set({
    name: data.name,
    slug: data.slug,
    description: data.description,
    imageUrl: data.imageUrl,
    ...
    seoTitle: data.seoTitle,
    seoDesc: data.seoDesc,
  }).where(eq(collectionsTable.id, raw)).returning();
  if (!collection) {
    res.status(404).json({ error: "Collection not found" });
    return;
  }
