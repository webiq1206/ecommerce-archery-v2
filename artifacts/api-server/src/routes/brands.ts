...
import { CreateBrandBody, UpdateBrandBody, GetBrandParams, DeleteBrandParams } from "@workspace/api-zod";
...
router.get("/brands", async (_req, res): Promise<void> => {
  ...
  const counts = await db.select({ brandId: productsTable.brandId, count: sql<number>`count(*)::int` }).from(productsTable).where(sql`${productsTable.brandId} IS NOT NULL`).groupBy(productsTable.brandId);
...
router.post("/brands", async (req, res): Promise<void> => {
  ...
  const [brand] = await db.insert(brandsTable).values(parsed.data as any).returning();
...
router.put("/brands/:id", async (req, res): Promise<void> => {
  ...
  if (!parsed.success) { res.status(400).json({ error: parsed.error.message }); return; }
  const [brand] = await db.update(brandsTable).set(parsed.data as any).where(eq(brandsTable.id, raw)).returning();
  if (!brand) { res.status(404).json({ error: "Brand not found" }); return; }
  res.json(brand);
});
