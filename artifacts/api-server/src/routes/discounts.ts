...
import { CreateDiscountBody, UpdateDiscountBody, ValidateDiscountBody } from "@workspace/api-zod";
...
router.post("/discounts", async (req, res): Promise<void> => {
  ...
  const [discount] = await db.insert(discountsTable).values({
    code: data.code,
    type: data.type as "PERCENTAGE" | "FIXED_AMOUNT" | "FREE_SHIPPING",
    value: data.value,
...
router.put("/discounts/:id", async (req, res): Promise<void> => {
  ...
  const [discount] = await db.update(discountsTable).set({
    code: data.code,
    type: data.type as "PERCENTAGE" | "FIXED_AMOUNT" | "FREE_SHIPPING" | undefined,
    value: data.value,
    minOrderValue: data.minOrderValue,
    maxUses: data.maxUses,
    isActive: data.isActive,
    startsAt: data.startsAt ? new Date(data.startsAt) : undefined,
    expiresAt: data.expiresAt ? new Date(data.expiresAt) : undefined,
  }).where(eq(discountsTable.id, raw)).returning();
  if (!discount) {
...
router.post("/discounts/validate", async (req, res): Promise<void> => {
  ...
  if (!discount) {
    res.json({ valid: false, error: "Invalid discount code" });
    return;
  }
  if (!discount.isActive) {
    res.json({ valid: false, error: "Discount is inactive" });
    return;
  }
  if (discount.maxUses && discount.usedCount >= discount.maxUses) {
    res.json({ valid: false, error: "Discount has been fully redeemed" });
    return;
  }
  if (discount.expiresAt && new Date() > discount.expiresAt) {
    res.json({ valid: false, error: "Discount has expired" });
    return;
  }
  if (discount.startsAt && new Date() < discount.startsAt) {
    res.json({ valid: false, error: "Discount is not yet active" });
    return;
  }
